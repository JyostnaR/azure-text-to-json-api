...
var uniqueSuffix = substring(uniqueString(resourceGroup().id, environment), 0, 8)
var appName = '${appNamePrefix}-${environment}-${uniqueSuffix}'
var storageName = '${appNamePrefix}${environment}${uniqueSuffix}'
var keyVaultName = '${appNamePrefix}-${environment}-${uniqueSuffix}-kv'
...
output functionAppConnectionString string = 'DefaultEndpointsProtocol=https;AccountName=${storageAccount.name};EndpointSuffix=${az.environment().suffixes.storage};AccountKey=${storageAccount.listKeys().keys[0].value}'
output blobStorageConnectionString string = 'DefaultEndpointsProtocol=https;AccountName=${blobStorageAccount.name};EndpointSuffix=${az.environment().suffixes.storage};AccountKey=${blobStorageAccount.listKeys().keys[0].value}'
